...
def getScore (b : Board) (player : Turn) (t : Turn) (depth : Nat) : Int :=
  match depth with
  | 0 => b.evaluate player
  | .succ n =>
    if t = player then
      (b.possibleMoves t).foldl (fun acc m ↦
      if acc > bigScore then acc else
      max acc (getScore (b.applyMove m) player t.next n)) (-1000)
    else
      (b.possibleMoves t).foldl (fun acc m ↦
       if -acc > bigScore then acc else
       min acc (getScore (b.applyMove m) player t.next n)) 1000

/-- TODO Was wenn keine Moves -/
def Board.bestMove (b : Board) (t : Turn) (depth : Nat) : Move × Int :=
  (b.possibleMoves t).map (fun m ↦ (m,getScore (b.applyMove m) t t.next depth)) |>.maxOn? (fun mt ↦ mt.2) |>.get!

def TestBoard1 := FENtoBoard (parseFenString "4k3/2pp3p/4b3/1np1p1q1/1N1P3P/4BPP1/P3P3/R2QKBNR test")

--#time #eval Board.bestMove' TestBoard1 .Black 3
